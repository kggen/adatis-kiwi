...
ggplot(res1, aes(x=factor(month,levels=month.name), y=data)) + 
  geom_line() + geom_hline(aes(yintercept = mean(res$data)), color="red") +
  geom_bar(stat="identity", width=.5, fill=(values=c("#999999", "#E69F00", "#56B4E9"))) + 
  labs(caption="Red Line: 2016 Monthly Average") + 
  theme(axis.title.x=element_blank(),axis.title.y=element_blank()) +
  geom_text(aes(label = round(data), hjust = 0.5, vjust = 3))
